    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            if (var == "no"):
                ...
                update.effective_message.reply_text("Desabilita conexion con el grupo para usuarios")
            elif(var == "yes"):
                sql.set_allow_connect_to_chat(chat.id, True)
                update.effective_message.reply_text("Habilita conexion con el grupo para usuarios")
            else:
                update.effective_message.reply_text("Envia on/yes/off/no en el grupo!")
        else:
            update.effective_message.reply_text("Envia on/yes/off/no en el grupo!")
    else:
        update.effective_message.reply_text("Envia on/yes/off/no en el grupo!")
        ...
        if len(args) >= 1:
            ...
            if (bot.get_chat_member(connect_chat, update.effective_message.from_user.id).status in ('administrator', 'creator') or 
                ...
                if connection_status:
                    ...
                    update.effective_message.reply_text("Lujo! conectado a *{}*".format(chat_name), parse_mode=ParseMode.MARKDOWN)
                    ...
                    update.effective_message.reply_text("Conexion fallida, ups!")
            else:
                update.effective_message.reply_text("Conexion a ese grupo no permitida!")
        else:
            update.effective_message.reply_text("Pone el ID del grupo a conectar!")
            history = sql.get_history(user.id)
            print(history.user_id, history.chat_id1, history.chat_id2, history.chat_id3, history.updated)

    else:
        update.effective_message.reply_text("Eso es solo para mp!")
...
def disconnect_chat(bot, update):
    if update.effective_chat.type == 'private':
        ...
        if disconnection_status:
            sql.disconnected_chat = update.effective_message.reply_text("Desconectado!")
            #Rebuild user's keyboard
            keyboard(bot, update)
        else:
           update.effective_message.reply_text("Desconexion erronea, mierdas")
    else:
        update.effective_message.reply_text("Solo usado en mp")
        ...
        if (bot.get_chat_member(conn_id, user_id).status in ('administrator', 'creator') or 
            ...
            if need_admin == True:
                if bot.get_chat_member(conn_id, update.effective_message.from_user.id).status in ('administrator', 'creator') or user_id in SUDO_USERS:
                    ...
                    update.effective_message.reply_text("Tenes que ser admin para conectarte al grupo, idiota!")
                    exit(1)
            else:
                return conn_id
        else:
            update.effective_message.reply_text("Ok, o el grupo cambio la conexion, o ya no sos admin.\nTe desconecto.")
...
Actions are available with connected groups:
 • Ver y editar notas
 • Vver y editar filtros
 • Mas bobadas!

 - /connect <chatid>: Te conecta a un grupo por id
 - /disconnect: En serio lo explico?
 - /allowconnect on/yes/off/no: Permite a usuarios conectarse
"""
