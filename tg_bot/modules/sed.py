...
def sed(bot: Bot, update: Update):
    ...
    if sed_result and update.effective_message.reply_to_message:
        ...
        if not repl:
            update.effective_message.reply_to_message.reply_text("Intentas reemplazar... "
                                                                 "nada con algo?")
            return
        ...
        try:
            ...
            if check and check.group(0).lower() == to_fix.lower():
                update.effective_message.reply_to_message.reply_text("Hola a todos!, {} intenta hacerme "
                                                                     "decir cosas que no quiero "
                                                                     "Di!".format(update.effective_user.first_name))
                return
            ...
            update.effective_message.reply_text("Do you even sed? Apparently not. (idiota)")
            return

        # empty string errors -_-
        if len(text) >= telegram.MAX_MESSAGE_LENGTH:
            update.effective_message.reply_text("El resultado es muy largo para \
                                                 telegram!")
        elif text:
...
__help__ = """
 - s/<text1>/<text2>(/<flag>): Reemplaza un mensaje haciendo un "sed" en ese mensaje, reemplazando las \
...
eg: \\?. Esta en pruebas, y gracias JS (no usar)
