    ...
    if can_delete(chat, bot.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("bloqueados {} en mensajes para no admins!".format(args[0]))
                ...
                message.reply_text("Bloqueados {} para los no admins!".format(args[0]))
                return "<b>{}:</b>" \
                       "\n#LOCK" \
                       "\n<b>Admin:</b> {}" \
                       "\nLocked <code>{}</code>.".format(html.escape(chat.title),
                ...
                message.reply_text("Que intentas blqouear...? Proba /locktypes para ver la lista, idiota")

    else:
        message.reply_text("O no soy admin o alguien me quito mis derechos, quiero venganza.")
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, bot.id):
                            message.reply_text("Veo un bot, le dije que no entre... "
                                               "pero no soy admin!")
                            return

                        chat.kick_member(new_mem.id)
                        message.reply_text("Solo administradores pueden agregar bots! tomate el palo.")
            else:
                try:
                    message.delete()
...
__help__ = """
 - /locktypes: Lista de bloqueables

*Admin only:*
 - /lock <type>: bloquea tipos de mensajes (no en privado)
 - /unlock <type>: Lo mismo al contrario
 - /locks: Lo que esta bloqueado actualmente.

Se puede usar para restringir usuarios.
ej:
Bloqueando urls va a eliminar todos los mensajes que tengan una url que no este en la lista blanca, los stickers los elimina todos \
emojis, etc.
Locking bots evita que no admins agreguen bots.
"""
